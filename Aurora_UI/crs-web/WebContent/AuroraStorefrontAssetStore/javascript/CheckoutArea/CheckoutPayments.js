...
CheckoutPayments = {
	...
	*/
	populateProtocolData:function(formName,params){
		...
		//Cybersource start
		params = this.updateParamObject(params,"instrumentIdentifierId",this.getValue(formName,"instrumentIdentifierId"),true);
		//CyberSource end
	...
	*/
	...
	addPaymentInstructions: function(){
		...
		if(this.getPaymentsToAdd().length > 0) {
			//CyberSource start
			var paymentAreaNumber = this.getPaymentsToAdd()[this.getPaymentsToAdd().length-1];
			/**CyberSource Flex API is calling before the PIAdd */
			var form = document.forms["PaymentForm" + paymentAreaNumber];
			var payMethodId=this.getValue(form,"payMethodId");
			
			if(payMethodId=="CyberSource"){
				var isCyberSourceCall=this.getValue(form,"isCyberSourceCall");
				var instrumentIdentifierId=this.getValue(form,"instrumentIdentifierId");
				if(isCyberSourceCall == "false" && instrumentIdentifierId===""){
					this.generateFlexKey();
				
				}else{
					this.getPaymentsToAdd().pop();
	...
	// CyberSource start
	/**
	 * Generate Flex key for invoke the cybersource/flexkey API. return KeyId
	 * and Hostname URL
	 */
	generateFlexKey : function() {

		var params = [];
		params["storeId"] = this.storeId;
		params["catalogId"] = this.catalogId;
		params["encryptionType"] = "None";
		wcService.invoke('ajaxCyberSourceGenerateKey', params);
	},
	removeInstrumentIdentifier : function(i) {
		document.getElementById("instrumentIdentifierId_" + i).value = "";
	},
	/**
	 * Get the card image from card number
	 */
	getCardImage : function(cardId, brand, number) {
		var element = document.getElementById(cardId);
		var removeClass = element.classList[element.classList.length - 1];
		...
		document.getElementById(brand).value = addclass;
		element.classList.remove(removeClass);
		element.classList.add(addclass);
	},

	/**
	 * Find the cybersource cartype from card number
	 */
	cyberSourceCreditCardType : function(cc) {
		var
	...
	/**
	 * parse the cybersource tokenize service response
	 */
	cyberSourceFlexTokenResonse : function(response) {

		console.log("instrument identifier id:"
				+ response._embedded.icsReply.instrumentIdentifier.id);

		var id = response._embedded.icsReply.instrumentIdentifier.id;
		var paymentAreaNumber = CheckoutPayments.getPaymentsToAdd()[CheckoutPayments
				.getPaymentsToAdd().length - 1];
		var form = document.forms["PaymentForm" + paymentAreaNumber];

		form.instrumentIdentifierId.value = id;
		form.isCyberSourceCall.value = "true";
		CheckoutPayments.addPaymentInstructions();
	},

	/**
	 * call the Cybersource Tokenize Card service API return
	 * instrumentIdentifierId
	 */
	cyberSourceFlexToken : function(keyId, host) {
		var paymentAreaNumber = this.getPaymentsToAdd()[this.getPaymentsToAdd().length - 1];
		var form = document.forms["PaymentForm" + paymentAreaNumber];
		var options = {
			cardInfo : {
				cardNumber : this.getValue(form, "account1"),
				cardType : this.cyberSourceCreditCardType(this.getValue(form,
						"account1"))
			},
			keyId : keyId
		};
		var tokenURL = "https://" + host + "/flex/v1/tokens";
		$
				.ajax({
					url : tokenURL,
					type : 'post',
					...
					success : function(data, textStatus, jQxhr) {
						...
						CheckoutPayments.cyberSourceFlexTokenResonse(data);
					},
					error : function(jqXhr, textStatus, errorThrown) {
						CheckoutPayments.paymentsToAdd.pop();
						if (textStatus = 'error') {
							if (jqXhr.responseJSON != null) {
								if (jqXhr.responseJSON.responseStatus.details.length > 0) {
									MessageHelper
											.displayErrorMessage(jqXhr.responseJSON.responseStatus.details[0].message);
								} else {
									MessageHelper
											.displayErrorMessage(jqXhr.responseJSON.responseStatus.message);
								}
							} else {
								MessageHelper
										.displayErrorMessage(CheckoutPayments.errorMessages["ERROR_CYBERSOURCE_API"]);
							}
						}
						return;
