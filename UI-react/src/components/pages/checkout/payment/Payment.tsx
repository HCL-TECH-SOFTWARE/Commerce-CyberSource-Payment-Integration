...
const Payment: React.FC = (props: any) => {
  ...
  useEffect(() => {
    console.log("cart--",cart,addressDetails);
  ...
  /**Lunu POC */
  const [showLunu, setLunu] = useState<boolean>(false);
  ...
  function onSuccessTransactionGooglePayToken(payload) {
    console.log("GooglePay success --", payload);
    if (payload.details['billingAddress']) {
      updateBillingAddress(payload);
    } else {
      //setGooglePayPayload
  ...
  async function onSuccessTransactionOfLunu(payload) {
    ...
    console.log("Lunu Payload -- ", payload);
    var resLunu = {
    ...
    // let body: any = {
    //   piAmount: selectedPaymentInfoList[0].piAmount,
    //   billing_address_id: selectedPaymentInfoList[0].addressId,
    //   payMethodId: selectedPaymentInfoList[0].payMethodId
    // };
    ...
    // await paymentInstructionService.addPaymentInstruction(payloadLunu);
  }
